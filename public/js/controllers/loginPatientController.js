function loginPatientController(userPatientService, sessionFactory, $timeout, $location, $rootScope) {
    ...
    this.login = () => {
        this.userPatientService.connect({
            email: this.email,
    ...
    this.createAccount = () => {
        this.userPatientService.create({
            email: this.email,
            password: this.password
        }).then((res) => {
            this.sessionFactory.token = res.data.token;
